...
function Create() {
    ...
    const handleSubmit =(e) => {
        e.preventDefault();
        axios.post("http://localhost:3000/users", items)
        .then(res => {
            navigate('/')
        })
        .catch(err => console.log(err))
       ...
       <form className='border-3 w-1/3 the-form shadow-2xl rounded-md' onSubmit={handleSubmit}  >
        ...
        <div className="input-data">
           ...
           <div className="label-input">
                ...
                <input type="email" placeholder='abc@gmail.com' value={items.email} onChange={(e) => setItems({...items,email: e.target.value})}/>
           </div>
        </div>
        <div className="btn">
            <button  className='rounded-md bg-green-600 text-white mr-3' type='submit'>Submit</button>
            <Link to='/'>
